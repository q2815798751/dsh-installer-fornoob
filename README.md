...
- **双击** 或选「显示 / 隐藏窗口」→ 恢复窗口；
- 右键菜单：启动后端 / 打开网页 / 关闭后端 / 退出；发现面板有新版本时，
  最上面会多一项「更新启动器到 vX」。
...
启动时会**静默检查一次**（查不到就什么都不说，不弹窗、不打扰）。发现有新版时：

- 面板右下角从 `v1.5.7 · DSH` 变成 `v1.5.7 → v1.5.8 可更新`，「检查更新」按钮也跟着变，
  托盘右键菜单多一项「更新启动器到 v1.5.8」——**不用先打开窗口就能发现**；
- 点进去（或窗口顶部的横幅）有一个 **更新启动器** 按钮：下载 → 校验 → 就地替换 →
  **自动重启**，窗口自己关掉，几秒钟后新面板已经在跑。**后端不受影响，全程不用你动手。**

替换失败的代价是「还是原来那个版本」，不会留下装坏的面板；万一自动重启没成功，
窗口会留着并告诉你怎么手动重开。
...
**Q：面板启动器好像不见了 / 双击提示找不到文件？**
自更新过程中被强行中断（断电、杀进程）可能留下这个状态：好文件其实在
`launcher\DSHLauncher.old.exe`。**把 `DSHLauncher.old.exe` 改名成 `DSHLauncher.exe`**
即可恢复。更新失败时窗口和 `launcher\data\update.log` 都会写明这一点。

**Q：面板更新提示了，但我不想更新？**
不点就行。提示只是面板右下角一行字和菜单里一项，不会自动下载、不会自动重启，
更不会弹窗。想彻底不看：忽略它即可，它每次启动只会多花一次只读的版本查询。
...
# 更新链路自检（拉 npm 版本列表 + 跑一遍环境检查 + 查一次面板更新，不写盘）
python launcher\launcher.pyw --selftest-update

# 面板自更新自检（真实下载本仓库的最新 DSHLauncher.exe、真实校验，并在临时目录里
# 对一个副本做真实替换——不动你正在用的面板）。更新失败时跑它，把输出发出来）
python launcher\launcher.pyw --selftest-launcher-update

# 重启自检（真的释放单例、起新进程、确认它接住端口，然后收尾）
python launcher\launcher.pyw --selftest-restart
...
- **启动器自更新**：查本仓库的 release，下载新的 `DSHLauncher.exe`。Windows
  ...
  改名成 `.old` → 新 exe 就位 → 自动重启 → 下次启动时删掉 `.old`」。重启前会先
  释放单实例端口，否则新进程会以为已经有一个启动器在跑；起完新进程后旧进程还会
  观察它 2.5 秒，**确认新面板真的活着才退出** —— 否则「面板消失」就成了没有退路的故障。
- **启动时的那次检查是只读的**：一次 GitHub API 请求（直连优先，直连不通才用系统
  代理），失败一律静默 —— 断网、被墙、接口限流都不会有任何提示或延迟。它也不缓存、
  不重试、不自动下载。
